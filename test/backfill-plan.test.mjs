...
test("plans one deterministic R2 publication per job, author, and community", () => {
  ...
  assert.deepEqual(result.estimate, { publications: 3, queueOperations: 6, d1Rows: 30, r2Bytes: result.estimate.r2Bytes });
});

test("fails closed before exceeding the internal free-tier publication ceiling", () => {
  const jobs = Array.from({ length: 2_001 }, (_, index) => ({ ...job, id: `gh_${index}`, contentHash: String(index).padStart(64, "0") }));
